...
from ..lib._utils import get_relion_particles_table, parse_relion_image_name_1based, read_star_any
...
class MicrographMembraneSubtract:
    def __init__(
        ...
        *,
        ...
        star_tables_particles = read_star_any(self.particles_selected_filename)
        ...
        # A "task" is one (particle stack, micrograph) pair (assumes 1 stack per micrograph).
        self.rawimage_stacks_name_lst = self.get_rawimage_stacks_name_lst()
        self.raw_mg_name_lst = self.get_raw_stack_micrograph_pairs()

        # Weighting mask is derived from particle box size; lazily initialised per-box-size.
    ...
    def process_micrograph_mem_subtract(self, raw_mg_name):
        ...
        # Dependency `.mxt` gating (Spec v1 §10.3).
        ...
        if self.require_particle_mxt:
            ...
            if dep.get("task") != "radonfit_particle_pms" or dep.get("status") != "success":
        ...
        try:
            ...
            # Box-size-derived weighting mask (Spec v1 §3.2-C).
    ...
    def micrograph_mem_subtract_multiprocessing(self, num_cpus, batch_size):
        try:
            ...
            # Start method already set (safe to continue).
            pass
        def chunks(lst, n):
